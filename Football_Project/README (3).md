...
> Validated across three leagues covering the full spectrum of modern football — including a fully out-of-sample blind test on Bundesliga 24/25.
...
| Liga | Type | Sniper Accuracy | Coverage | Brier (Sniper) | Karakteristik |
|------|:----:|:-:|:-:|:-:|---|
| La Liga 🇪🇸 | In-sample backtest | **75.0%** | 30% | 0.2002 | Low-scoring / Posesif |
| EPL 🏴󠁧󠁢󠁥󠁮󠁧󠁿 | Rolling backtest | **62.1%** | 28% | 0.2073 | High-intensity / Chaos |
| Bundesliga 🇩🇪 | Backtest (22/23→23/24) | **68.4%** | 32% | 0.4869 | High-scoring / Transisi |
| Bundesliga 🇩🇪 | **Blind test (24/25)** | **65.8%** | 29% | 0.4796 | Out-of-sample validation |

> Bundesliga Brier lebih tinggi dari La Liga/EPL karena overconfidence bias pada tim besar di musim anomali (Leverkusen invincible 23/24) dan variance gol tinggi (~3.2 gol/laga). Dokumentasi lengkap di [Failure Analysis](#failure-analysis).
>
> **Key finding:** Backtest (68.4%) vs Blind test (65.8%) — deviasi hanya 2.6%, membuktikan temporal robustness model tanpa data leakage.

---

## Evaluation Strategy

Model divalidasi menggunakan **tiga metodologi independen** untuk memastikan tidak ada data leakage atau overfitting:

1. **Walk-forward backtest** — Train pada musim N, test pada musim N+1, update Elo setelah setiap prediksi. Tidak ada look-ahead bias.
2. **Rolling backtest** — EPL diuji selama 2 musim penuh (760 laga) dengan sliding window.
3. **Out-of-sample blind test** — Model di-freeze setelah training, lalu dijalankan pada Bundesliga 24/25 yang **belum pernah dilihat model sama sekali**. Parameter tidak disentuh setelah training selesai.

> *"To ensure the model's temporal robustness, a final out-of-sample test was conducted on the 24/25 Bundesliga season. This serves as a real-world simulation — testing whether parameters calibrated on the 22/23–23/24 seasons retain their predictive power in a new competitive landscape (new promoted teams: Holstein Kiel, St. Pauli; post-Leverkusen-dynasty dynamics)."*
...
Validation   : Walk-forward backtest + out-of-sample blind test (no data leakage)

La Liga      : 75.0% accuracy | 30% coverage | Brier 0.2002 | n≈90  laga
EPL          : 62.1% accuracy | 28% coverage | Brier 0.2073 | n=213 laga
Bundesliga   : 68.4% accuracy | 32% coverage | Brier 0.4869 | n=76  laga [backtest]
Bundesliga   : 65.8% accuracy | 29% coverage | Brier 0.4796 | n=79  laga [blind test ✓]
               Backtest→Blind drift: -2.6pp — temporal robustness confirmed
```
